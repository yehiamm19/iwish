...
| 22100792 | Ali Khaled Mohamed | MySQL database and JDBC persistence | `src/iwish/server/Database.java`, `database/iwish-schema.sql`, `database/iwish-backup.sql`, `database/iwish-seed.sql` | Explain `Connection`, `Statement`, `PreparedStatement`, `ResultSet`, foreign keys, transactions, SHA-256 password hashing, schema creation, loading data, saving data, and why the application does not connect to MySQL directly from the client. Demonstrate the corresponding rows in phpMyAdmin. |
...
| 23101614 | Youssef Ramdan | Wish-list business rules, contributions, notifications, and delivery integration | `src/iwish/server/IWishServer.java` methods `addItem`, `updateItem`, `deleteItem`, `contribute`, `readNotifications`, `src/iwish/client/JavaFxApp.java` wish/contribution/notification methods, `pom.xml`, `build.xml`, `nbproject/` | Explain wish CRUD, remaining-balance validation, funded-state transitions, buyer and receiver notifications, Maven and Ant integration, JavaFX/Connector/J dependencies, and the complete demonstration path. Demonstrate an item becoming fully funded and show the generated notifications. |
